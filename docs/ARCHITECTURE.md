...
Focused evidence packets are projections, not snapshot serialization. The domain builder selects one current finding or attack path, copies its lifecycle and interpretation fields, and includes only referenced objects and relationships with exact source evidence. It deliberately excludes arbitrary node metadata, credential material, tokens, and unrelated tenant inventory. The authenticated route reuses the export audit boundary and performs a tenant check before attaching the current snapshot-scoped review. Markdown escapes tenant-controlled text; JSON uses the versioned `ere-evidence-packet/1.0` contract.
