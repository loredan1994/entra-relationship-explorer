...
export async function GET(request: NextRequest) {
  ...
  const findings = intelligence.findings.map((finding) => {
    ...
    return `<article><div class="meta"><b class="${e(finding.severity)}">${e(finding.severity.toUpperCase())}</b><span>${e(finding.evidenceClass)} evidence</span><span>${e(finding.category.replaceAll("-", " "))}</span></div><h2>${e(finding.title)}</h2><p>${e(finding.summary)}</p><h3>Why it matters</h3><p>${e(finding.whyItMatters)}</p><h3>Recommended action</h3><ol>${finding.remediation.map((item) => `<li>${e(item)}</li>`).join("")}</ol>${decision}<details><summary>Evidence references</summary><p><strong>Objects:</strong> ${e(finding.affectedObjectIds.join(", ") || "None")}</p><p><strong>Edges:</strong> ${e(finding.edgeIds.join(", ") || "None")}</p><p><strong>Endpoints:</strong> ${e(finding.sourceEndpoints.join(", ") || "None")}</p>${finding.uncertainty.map((item) => `<p>${e(item)}</p>`).join("")}</details></article>`;
  }).join("");
