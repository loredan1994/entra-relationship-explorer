  ...
  return <>
    ...
    <div className="threat-workspace">
      ...
      <section className="finding-detail" aria-live="polite">{selected ? <>
        <header className="finding-detail-header"><div><span className={`severity-pill severity-${selected.severity}`}>{selected.severity}</span><span className={`evidence-chip evidence-${selected.evidenceClass}`}>{evidenceLabel[selected.evidenceClass]}</span><span className={`lifecycle-chip lifecycle-${selectedLifecycle}`}>{selectedLifecycle}</span>{selected.rule ? <span className="rule-chip" title={`${selected.rule.title}, version ${selected.rule.version}`}>{selected.rule.id} · v{selected.rule.version}</span> : null}<h2>{selected.title}</h2><p>{selected.summary}</p></div><div className="detail-actions"><Link className="button button-secondary" href={`/api/export/findings.csv?finding=${encodeURIComponent(selected.id)}`}>Export CSV</Link><Link className="button button-secondary" href="/api/export/report.html">Client report</Link>{selectedPath ? <Link className="button button-secondary" href={`/api/export/attack-flow.json?path=${encodeURIComponent(selectedPath.id)}`}>Attack Flow</Link> : null}{selected.edgeIds[0] ? <Link className="button button-primary" href={`/map?edge=${encodeURIComponent(selected.edgeIds[0])}`}>Inspect evidence</Link> : null}</div></header>
